...
export default class DashboardTrip extends Component {
    ...
    componentDidUpdate = (prevProps, prevState) => {
        if(this.props.modalVisible !== prevProps.modalVisible) {
            ...
            UserController.request.tripInfo(modalData.tripId, modalData.cid)
            .then(res => {
                const coor = this.getCoordinates(res.data.coordinates, res.data.trips);
                ...
                const tripMap = res.data.trips.map((trip, index) => {
                    const coor = {
                    ...
                    } else if(index === (res.data.trips.length - 1)) {
                ...
                const firstTrip = res.data.trips.find((x, idx) => idx === 0);
                const lastTrip = res.data.trips.find((x, idx) => idx === (res.data.trips.length - 1));
                const tripTimeStamp = {
    ...
    getCoordinates = (dataMap, trips) => {
        ...
        const coordinates = dataMap.map(c =>
            JSON.parse(c.json_coordinates).map(geo =>
                geo.map(coord => {
                    lats.push(coord.lat);
                    lons.push(coord.lng);
                    const coor = {
                                ...
                                {this.state.loader ? (
                                    ...
                                    <View>
                                        ...
                                        >
                                            ...
                                            >
                                                {this.state.coordinates.map((polygon, index) =>
                                                    polygon.map(c =>
                                                        <Polygon
                                                            key={index}
                                                            coordinates={c}
